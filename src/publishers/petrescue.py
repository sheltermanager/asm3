    ...
    def run(self):
        ...
        for an in animals:
            try:
                ...
                hwtreated = isdog and self.dbo.query_int("SELECT COUNT(*) FROM animalmedical WHERE LOWER(TreatmentName) LIKE '%heart%' " \
                    "AND LOWER(TreatmentName) LIKE '%worm%' AND AnimalID=?", [an.ID]) > 0
