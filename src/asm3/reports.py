...
def get_ask_animal_reports(dbo: Database, superuser: int, roleids: str):
    filteredreports = []
    allreports = dbo.query(f"SELECT ID, Title, Category, Revision FROM customreport WHERE %s ORDER BY Title" % dbo.sql_ilike("SQLCommand", "?"), ['%$ask animal$%'])
...
def get_ask_person_reports(dbo: Database, superuser: int, roleids: str):
    filteredreports = []
    allreports = dbo.query(f"SELECT ID, Title, Category, Revision FROM customreport WHERE %s ORDER BY Title" % dbo.sql_ilike("SQLCommand", "?"), ['%$ask person$%'])
    roles = dbo.query("SELECT * FROM customreportrole")
    for report in allreports:
        viewroleids = []
        for role in roles:
