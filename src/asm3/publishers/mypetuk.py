...
from asm3.sitedefs import MYPET_BASE_URL, MYPET_API_USER, MYPET_API_KEY
    ...
    def run(self) -> None:
        ...
        for an in animals:
            try:
                ...
                authheaders = {
                    "APIUSER": MYPET_API_USER,
                    "APIKEY": MYPET_API_KEY
                }

                try:
                    # Post the VetXML document
                    self.log("Posting microchip registration document to %s \n%s\n" % (MYPET_BASE_URL, x))
                    r = asm3.utils.post_xml(MYPET_BASE_URL, x, authheaders)
        ...
        # Only mark processed if we aren't using MyPet staging URL
        if len(processed_animals) > 0 and MYPET_BASE_URL.find("staging") == -1:
            self.log("successfully processed %d animals, marking sent" % len(processed_animals))
            self.markAnimalsPublished(processed_animals)

        if MYPET_BASE_URL.find("staging") != -1:
            self.log("MyPet staging mode, not marking animals published")
