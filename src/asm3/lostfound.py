...
def send_email_from_form(dbo: Database, username: str, post: PostedData) -> bool:
    ...
    if addtolog == 1:
        asm3.log.add_log_email(dbo, username, post["lfmode"] == "lost" and asm3.log.LOSTANIMAL or asm3.log.FOUNDANIMAL, post.integer("lfid"), logtype, emailto, subject, body, emailcc, emailbcc)
    return rv
...
def create_animal_from_found(dbo: Database, username: str, aid: int) -> int:
    ...
    data = {
        ...
        "breed1":               a["BREEDID"],
        "breed2":               a["BREEDID"],
        "reasonforentry":       _("Found in {0} {1}").format(a["AREAFOUND"], a["AREAPOSTCODE"]),
