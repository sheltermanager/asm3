...
def get_animalcontrol_for_animal(dbo: Database, username: str, aid: int) -> Results:
    """ Return the list of linked incidents for an animal """
    ac = dbo.query(get_animalcontrol_query(dbo) + \
        " INNER JOIN animalcontrolanimal aca ON aca.AnimalControlID = ac.ID " \
        "WHERE aca.AnimalID = ? ORDER BY IncidentDateTime DESC", [aid])
    return reduce_find_results(dbo, username, ac)
...
def send_email_from_form(dbo: Database, username: str, post: PostedData) -> bool:
    ...
    if addtolog == 1:
        asm3.log.add_log_email(dbo, username, asm3.log.ANIMALCONTROL, post.integer("incidentid"), logtype, emailto, emailcc, emailbcc, subject, body)
    return rv
...
def clone_animalcontrol(dbo: Database, username: str, acid: int) -> int:
    ...
    dbo.insert("animalcontrol", {
        "ID":                   nid,
        "IncidentCode":         calc_incident_code(dbo, nid, dbo.now()),
        "IncidentDateTime":     dbo.now(),
        "IncidentTypeID":       ac.INCIDENTTYPEID,
        "CallDateTime":         dbo.now(),
        "CallNotes":            "",
