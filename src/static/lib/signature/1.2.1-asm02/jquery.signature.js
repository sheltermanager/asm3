/*! http://keith-wood.name/signature.html
    ...
    so that touchpunch is no longer needed */
...
$(selector).signature({color: 'blue', guideline: true}) */
  ...
  console.log('Signature changed');
		...
		/** Start a new line.
			@memberof Signature
			@private
			@param {Event} event The triggering event, needs clientX/clientY values. */
		_lineStart: function(event) {
		...
		/** End a line.
			...
			@private
			@param {Event} event The triggering event, needs clientX/clientY values. */
		_lineStop: function(event) {
			if (this.curLine.length === 1) {
				event.clientY += this.options.thickness;
				this._lineDraw(event);
			}
			this.lastPoint = null;
			this.curLine = null;
			this._changed(event);
