...
matched_route.run();return true;}else{if(Path.routes.rescue!==null){Path.routes.rescue(passed_route);}}}},'reload':function(path){if(!path){path=Path.route.current;}
var matched_route=Path.match(path,true);if(matched_route){matched_route.run();}},'listen':function(){var fn=function(){Path.dispatch(location.hash);}
