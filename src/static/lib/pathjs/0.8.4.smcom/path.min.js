...
Path.listen();}}}},'match':function(path,parameterize){var params={},route=null,possible_routes,slice,i,j,compare,qs;var splitqs=function(a){var i=0,b={};if(!a){return{};}
$.each(a.split("&"),function(i,v){var x=v.split("=");b[x[0]]=decodeURIComponent(x[1].replace(/\+/g," "));});return b;};if(path.indexOf("?")!=-1){qs=path.substring(path.indexOf("?")+1);path=path.substring(0,path.indexOf("?"));qs=splitqs(qs);}
...
if(slice===compare){if(parameterize){route.params=params;route.qs=qs;}
return route;}}}}
...
if(matched_route!==null){matched_route.run();return true;}else{if(Path.routes.rescue!==null){Path.routes.rescue();}}}},'listen':function(){var fn=function(){Path.dispatch(location.hash);}
