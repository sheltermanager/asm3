...
var Path={'version':"0.8.4.smcom",'map':function(path){if(Path.routes.defined.hasOwnProperty(path)){return Path.routes.defined[path];}else{return new Path.core.route(path);}},'root':function(path){Path.routes.root=path;},'rescue':function(fn){Path.routes.rescue=fn;},'change':function(fn){Path.routes.change=fn;},'history':{'pushState':function(state,title,path){if(Path.history.supported){if(Path.dispatch(path)){history.pushState(state,title,path);}}else{if(Path.history.fallback){window.location.hash="#"+path;}}},'popState':function(event){if(!event.state){return;}
Path.dispatch(Path.path_to_route());},'listen':function(fallback){Path.history.supported=!!(window.history&&window.history.pushState);Path.history.fallback=fallback;if(Path.history.supported){window.onpopstate=Path.history.popState;}else{if(Path.history.fallback){for(route in Path.routes.defined){if(route.charAt(0)!="#"){Path.routes.defined["#"+route]=Path.routes.defined[route];Path.routes.defined["#"+route].path="#"+route;}}
...
matched_route.run();return true;}else{if(Path.routes.rescue!==null){Path.routes.rescue(passed_route);}}}},'reload':function(){var path=Path.path_to_route();var matched_route=Path.match(path,true);if(matched_route){matched_route.run();}},'path_to_route':function(){var path=document.location.pathname;if(path.lastIndexOf("/")!=-1){path=path.substring(path.lastIndexOf("/")+1);}
if(document.location.search){path+=document.location.search;}
return path;},'listen':function(){var fn=function(){Path.dispatch(location.hash);}
