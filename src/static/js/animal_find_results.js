    ...
    const animal_find_results = {
        ...
        column_label: function(name, add) {
            const labels = {
                ...
                "IsGoodWithElderly":  _("Good with Elderly"),
                "IsHouseTrained":  _("Housetrained"),
                "IsCrateTrained":  _("Crate Trained"),
                "IsGoodTraveller":  _("Good Traveller"),
        ...
        format_column: function(row, name, value, add) {
            ...
            if (name == "Adoptable") {
                ...
                // and trigger different sorting rules. We prefix them all with an X
                // so that they force an alphanumeric rather than numeric sort
                rv = '<span data-sort="X' + html.title(row.CODE) + '">' + row.CODE + '</span>';
            }
