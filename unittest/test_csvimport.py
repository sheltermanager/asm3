...
class TestCSVImport(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        base.execute("DELETE FROM animal WHERE AnimalName = '%s'" % 'TestioCSV')
        base.execute("DELETE FROM diary WHERE Subject = '%s'" % 'Test diary note from csv import')
        base.execute("DELETE FROM owner WHERE OwnerName = '%s'" % 'Sir Bob Hoskins')
        base.execute("DELETE FROM owner WHERE OwnerName = '%s'" % 'Baron Barry Gibb')
        base.execute("DELETE FROM animalcontrol WHERE CallNotes = '%s'" % 'This incident was created as part of the CSV import unit test.')
        base.execute("DELETE FROM ownerlicence WHERE Comments = '%s'" % 'This license was created as part of the CSV import unit test.')
        base.execute("DELETE FROM stocklevel WHERE Description = '%s'" % 'This stock level was created as part of the CSV import unit test.')
        base.execute("DELETE FROM animalcost WHERE Description = '%s'" % 'This cost was created by the csvimport unit test.')
        base.execute("DELETE FROM log WHERE Comments = '%s'" % 'This log was created by the CSV Import unit test.')
        base.execute("DELETE FROM animalmedical WHERE Comments = '%s'" % 'This regimen was created by the CSV Import unit test.')
        base.execute("DELETE FROM adoption WHERE Comments = '%s'" % 'This movement was created by the CSV Import unit test.')
        base.execute("DELETE FROM animaltest WHERE Comments = '%s'" % 'This test was created by the CSV Import unit test.')
